...
pub fn initialize<'a>(einheiten : &Vec<&'a str>, daten : &mut Vec<HashMap<&'a str, f64>>, csv_data : &'static str) -> Result<()> {
...
pub fn bestimme_naeherung(umwerter : &Umwerter, quell_einheit : &str, wert : f64) -> Result<(f64, usize, f64, usize)> {
    let (mut ausgangsbereich_unten, mut zeile_untere_naeherung) = kleinstes_element(umwerter, quell_einheit);
    let (mut ausgangsbereich_oben, mut zeile_obere_naeherung) = groesstes_element(umwerter, quell_einheit);
    ...
    Ok((ausgangsbereich_unten, zeile_untere_naeherung, ausgangsbereich_oben, zeile_obere_naeherung))
}

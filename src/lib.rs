...
pub fn werte_um<'a>(wert : f64, externe_source_einheit : &str, externe_ziel_einheit : &str, tabelle : &'a UmwertungsTabelle)
               -> Result<f64> {
    ...
    let interne_source_einheit = match trait_tabelle.konvert_einheit(externe_source_einheit) {
        ...
        None => return Err(ErrorKind::QuellEinheitNichtVorhanden(externe_source_einheit.to_string()).into()),
    };
    
    let interne_ziel_einheit = match trait_tabelle.konvert_einheit(externe_ziel_einheit) {
        Some(einheit) => einheit,
        None => return Err(ErrorKind::ZielEinheitNichtVorhanden(externe_ziel_einheit.to_string()).into()),
    };

    let (naeherung_unten_wert, zeile_unten_wert, naeherung_oben_wert, zeile_oben_wert) = tools::bestimme_naeherung(&mut *trait_tabelle, interne_source_einheit, wert)?;
    ...
    Ok((ziel_oben_wert - ziel_unten_wert) / (naeherung_oben_wert - naeherung_unten_wert) * (wert - naeherung_unten_wert) + ziel_unten_wert)
}
